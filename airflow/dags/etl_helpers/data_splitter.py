...
def preprocess_for_split(df):
    ...
    try:
        ...
        if duplicates_removed > 0:
            logger.info(f"Removed {duplicates_removed} duplicate records ({100*duplicates_removed/initial_count:.2f}%)")
        else:
        ...
        if 'location_description' in df_clean.columns:
            ...
            if null_count > 0:
                ...
                logger.info(f"Filled {null_count} null values in 'location_description' with 'UNKNOWN'")
            else:
...
def split_train_test(df, test_size=0.2, random_state=42, stratify_column='arrest'):
    ...
    try:
        ...
        if stratify_column not in df.columns:
            logger.warning(f"Stratify column '{stratify_column}' not found. Performing random split.")
            stratify_data = None
        else:
        ...
        if stratify_data is not None:
            ...
            for value in train_dist.index:
                logger.info(f"  {value} - Train: {100*train_dist[value]:.2f}%, Test: {100*test_dist[value]:.2f}%")
